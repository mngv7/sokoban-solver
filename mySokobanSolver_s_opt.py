...
class SokobanPuzzle(search.Problem):
    ...
    def __init__(self, warehouse):
        ...
        self.reachable_cache = {}  # For caching reachable positions
        self.h_cache = {}          # For caching heuristic values
    ...
    def __hash__(self):
        return hash(self.initial)  # only hash based on the initial state
    ...
    def actions(self, state):
        # Early exit if pushing into taboo deadlock
        ...
        # Cache reachable positions based on current worker and boxes.
        ...
        for action in directions:
            ...
            if (nx, ny) in boxes_xy:
                ...
                if (bnx, bny) in self.walls or (bnx, bny) in boxes_xy:
                    continue
                # If the pushing position is reachable by the worker
    ...
    def result(self, state, action):
        ...
        for i, (bx, by, w) in enumerate(boxes):
            if (bx, by) == new_worker:
                ...
                boxes[i] = new_box
                break
        # Ensure the boxes remain sorted for state consistency.
    ...
    def h(self, node):
        state = node.state
        # Cache the computed heuristic value to avoid redundant computations.
...
def check_elem_action_seq(warehouse, action_seq):
    ...
    @return:
        ...
        a string representing the state of the puzzle after applying the sequence
        of actions. This must be the same string as the string returned by 
        Warehouse.__str__()
    '''
    ...
    for action in action_seq:
        ...
        if (new_worker_x, new_worker_y) in boxes:
            ...
            # If there's a wall or another box in the way.
...
def solve_weighted_sokoban(warehouse):
    ...
    It returns two items. The first item is an action sequence solution.
    The second item is the total cost of this action sequence.
    
    @param warehouse: a valid Warehouse object

    @return:
        If the puzzle cannot be solved: return 'Impossible', None
        If a solution was found: return S, C 
           where S is a list of actions that solves the puzzle coded with
           'Left', 'Right', 'Up', 'Down'
           If the puzzle is already in a goal state, simply return [].
           C is the total cost of the action sequence.
    '''
